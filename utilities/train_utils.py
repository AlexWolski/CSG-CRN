...
def load_model(num_prims, num_shapes, num_operations, device, args, model_params=None):
	...
	model = CSG_CRN(
		...
		device,
		args.encoder_layers,
		args.encoder_trans_conv_layers,
		args.encoder_trans_fc_layers,
		args.prim_decoder_layers,
		args.regressor_layers,
...
def init_training_params(training_logger, data_splits, args, device, model_params=None):
	...
	if not (train_dataset := PointDataset(train_split, device, args, augment_data=args.augment_data, loss_sampling_method=args.loss_sampling_method, dataset_name="Training Set")):
		return

	if not (val_dataset := PointDataset(val_split, device, args, augment_data=False, loss_sampling_method=args.loss_sampling_method, dataset_name="Validation Set")):
		return
