...
class Loss(nn.Module):
	...
	TARGET_RECON_SAMPLING_RATIO = 0.5
	...
	def forward(self, target_near_surface_samples, target_uniform_samples, target_surface_samples, csg_model):
		...
		if self.loss_sampling_method == self.UNIFIED_SAMPLING:
			target_uniform_samples = self._select_near_surface_samples(target_uniform_samples, csg_model)
	...
	def _select_near_surface_samples(self, target_uniform_samples, csg_model):
		target_points = target_uniform_samples[..., :3]
		target_distances = target_uniform_samples[..., 3]
		num_near_surface_recon_samples = math.ceil(self.num_loss_samples * Loss.TARGET_RECON_SAMPLING_RATIO)
		...
		(_, near_surface_points, near_surface_distances) = select_nearest_samples(target_points, combined_distances, num_near_surface_recon_samples)
